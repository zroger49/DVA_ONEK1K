...
# @Description: Code to compute the dispersion per celltype across genes, with permuted cells. Run this to check the quality of the analysis
# @software version: R=4.4

#### OneK1K data set with Azimith annotation ####
...
out.dir <- paste0(main.dir, "15_individual_dispersions_cell_permutations/")
...
ok1k@meta.data$permutedIndividual <- sample(ok1k@meta.data$assignment)

# Extract a list of individual IDs ----
a <- unlist(table(ok1k@meta.data$permutedIndividual))
...
for (k in 1:length(ind)){
  ...
  ss <- subset(x = ok1k, subset = permutedIndividual == as.character(ind[as.numeric(k)])) # Extract data from current individual
...
# Save result
write.table(res, paste0(out.dir, ct_name, "_within_ind_dispersion_", iter, ".txt"), row.names=T, col.names=T, quote=F)
